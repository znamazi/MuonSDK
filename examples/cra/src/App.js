...
function App() {
  ...
  const [resultCall, setResultCall] = React.useState([])
  ...
  React.useEffect(() => {
    const fetchData = async () => {
      ...
      setResultCall(JSON.stringify(resultCall, undefined, 2))
    }
    fetchData()
  }, [])
  ...
  return (
    <div className="App">
      <h2>Signatures Reponse Add Bridge When Is Used In React :</h2>
      <pre id="json">{result}</pre>
      <h2>Signatures Reponse Call Contract When Is Used In React :</h2>
      <pre id="json">{resultCall}</pre>
    </div>
  )
}
